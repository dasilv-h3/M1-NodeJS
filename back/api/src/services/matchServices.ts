...
export const getAllMatchesMasculinJunior = async (): Promise<Match[]> => {
    const [rows] = await pool.query('SELECT m.score, t.name AS team_name, s.name AS section_name, m.date FROM matches m INNER JOIN teams t ON m.opponent_id = t.id INNER JOIN sections s ON m.section_id = s.id WHERE m.section_id = 1 AND m.opponent_id= 1;');
    return rows as Match[];
};
